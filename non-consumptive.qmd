...
Especially as more platforms, outlets and individuals try to block their content from being scraped by AI companies --
and de facto everyone else.[^data]
These problems have been recognised in the field and are discussed as important challenges at methodological and issue specific conferences[^comptext] as well as in specialized working groups like the EDMO Working Group on Access to Platform Data[^edmo]
...
Most textual data is copyrighted, and moreover for some data (such as interpersonal communication)
there are privacy and other ethical concerns [@AtteveldtPrivates2021].
...
However, first we need to highlitght another setting that can modify access to data in `amcat4`: the authentication mode.
...
### Non-Consumptive Research via `actioncat`: creating and analysing annotations
